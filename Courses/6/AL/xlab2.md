...
<!-- , though it is a good exercise to do it yourself. 

::::::Exercise (force-equation)
Find the equation for the force, $F$, on the glider as a function of block height, $b$ and mass, $m$. You may need to consult the Math Sheets in the Appendix. 
:::Note (|12)
To get to the force, $\theta$ must first be determined. The angle $\theta$ can be obtained by using $b$ and $D$, where $b$ is the height of
the block and $D$ is the distance between the airtrack's legs. Once $\theta$ has been determined, use what you know about vectors to calculate the force.
:::
:::::: -->

<!--The first quantity of concern is the force, $F$. Examine [](#Figure-setup)a. The glider will glide down the ramp under the force of gravity. The strength of gravity is determined by the tilt of the track, which is in turn decided by the height of the block, $b$, that is placed under one of the legs.
